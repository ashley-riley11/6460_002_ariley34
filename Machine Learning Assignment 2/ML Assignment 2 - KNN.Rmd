...
groups <- dummyVars(~., data = df) # This creates the dummy vars

my_dummy.df <- as.data.frame(predict(groups,df))
```
...
#normalizing train, test, validate datasets
...
```{r}
#knn for each data set test/train/val using k = 3 as chosen above
...
These are determined by how we classified the data in the confusion matrix as number or true positive, true negative, false positive, false negative. We would expect the train set to perform the best since the model was trained using this data.  Here we can see that it does perform best.When we look at the test set confusion matrix, we know that this data was unseen to the model so we can closely examine how it performed compared to the train set. Here, we have a fairly close accuracy, so this indicates that our model performed well in this regard. The validation data also has never been seen by the model.  If we were to have a significantly lower accuracy on the validation set, we could have overfitting. Here, the validation set pretty closely matches the training set accuracy so, we have a well-fit model.**
